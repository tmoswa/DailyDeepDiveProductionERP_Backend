...
import com.softpala.SalesDeepDive_ERP.api.main.inc.increaments.IncreamentsServiceImplementation;
import com.softpala.SalesDeepDive_ERP.api.main.main_entity.MainEntityServiceImplementation;
import com.softpala.SalesDeepDive_ERP.persistence.dao.MainEntityRepository;
import com.softpala.SalesDeepDive_ERP.persistence.dao.RawMaterialsRepository;
import com.softpala.SalesDeepDive_ERP.persistence.dao.PurchaseOrderRepository;
...
public class PurchaseOrderServiceImplementation implements iPurchaseOrderService {
    ...
    public PurchaseOrder save(PurchaseOrder purchaseOrder,MultipartFile file) {
        ...
        RawMaterials ntm=ntMsRepository.findById(purchaseOrder.getNtMs().getId()).orElseThrow(() ->  new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("NTM not found")));
...
@Override
    ...
    if (details.getName().length() > 0) {
        ...
        if(purchaseOrder.getStatus().equals("Delivered") && details.getStatus().equals("Initiated")){
            RawMaterials ntMs=  ntMsRepository.findById(purchaseOrder.getNtMs().getId())
