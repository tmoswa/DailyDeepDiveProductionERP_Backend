...
import com.softpala.SalesDeepDive_ERP.api.main.material_usage.iMaterialUsageService;
...
public class ProductionRunMaterialUsageServiceImplementation implements iProductionMaterialUsageService {
    ...
    public ProductionMaterialUsage saveProductionMaterialUsage(ProductionMaterialUsage productionMaterialUsage) {
        RawMaterials details=      ntMsRepository.findById(productionMaterialUsage.getRawMaterialUsage().getId())
                .orElseThrow(() ->  new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("ntms with id %d not found", productionMaterialUsage.getRawMaterialUsage().getId())));

        double quantityNew=details.getQuantity()-productionMaterialUsage.getQuantity();
        //details.setQuantity(quantityNew);

        activityLogService.addActivityLog("Added Production Run Material Usage of : "+productionMaterialUsage.getRawMaterialUsage().getName() +" , of quantity "+productionMaterialUsage.getQuantity()+" , of Product: "+productionMaterialUsage.getProductionRun().getName(),"Production Run Material usage");
    ...
    public boolean deleteProductionMaterialUsage(Long id) {
        ...
        activityLogService.addActivityLog("Deleted Production Run Material Usage of : "+productionMaterialUsage.getRawMaterialUsage().getName() +" , of quantity "+productionMaterialUsage.getQuantity()+" , of Product: "+productionMaterialUsage.getProductionRun().getName(),"Production Run Material usage");
...
@Override
    ...
    if (details.getDescription().length() > 0) {
        ...
        RawMaterials NTMsDetails=      ntMsRepository.findById(productionMaterialUsage.getRawMaterialUsage().getId())
                .orElseThrow(() ->  new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("ntms with id %d not found", productionMaterialUsage.getRawMaterialUsage().getId())));
        ...
        activityLogService.addActivityLog("Updated Production Run Material Usage of : "+productionMaterialUsage.getRawMaterialUsage().getName() +" , from quantity "+details.getQuantity()+" , to quantity "+productionMaterialUsage.getQuantity()+" , of Product: "+productionMaterialUsage.getProductionRun().getName(),"Production Run Material usage");

        details.setRawMaterialUsage(productionMaterialUsage.getRawMaterialUsage());
