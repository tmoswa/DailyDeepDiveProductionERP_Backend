...
import com.softpala.SalesDeepDive_ERP.api.main.raw_materials.RawMaterialsServiceImplementation;
...
import com.softpala.SalesDeepDive_ERP.persistence.dao.MaterialCountRepository;
    ...
    public ResponseEntity<Response> updateMaterialCount(@PathVariable("id") Long id,@RequestBody MaterialCount[] materialCounts) {
        ...
        for(MaterialCount materialCount:materialCounts){
            ...
            MaterialCount materialCounts_0=materialCountRepository.findMaterialCountByMaterialStockCount(details)
                    .stream().filter(materialCount2->materialCount2.getRawMaterialUsage().getName().equals(materialCount.getRawMaterialUsage().getName())).findAny().get();

            materialCountServiceImplementation.updateMaterialCount(materialCounts_0.getId(),materialCount);

            }
/*
            Collection<MaterialCount> materialCounts1=  materialCountRepository.findMaterialCountByntMs_usage(materialCount.getRawMaterialUsage())
