...
import com.softpala.SalesDeepDive_ERP.api.config.currency.iCurrencyService;
...
@Transactional
@Slf4j
public class RawMaterialsServiceImplementation implements iRawMaterialsService {
    ...
    @Override
    public Collection<rawMaterialsUsed> ntmsList(int limit) {
        LocalDate startDate=LocalDate.parse("2023-08-01");
        LocalDate endDate=startDate.plusYears(3);
        Collection<rawMaterialsUsed> usedNTMs=this.ntmsUsedList(startDate, endDate, 1000000,false,new Product());
        List<RawMaterials> ntmsOpeningBalance=ntMsRepository.findAllByOrderBySequenceAsc();
        ...
        if(ld.isBefore(endDate)) {
            for (RawMaterials ntMsFin1 : ntmsOpeningBalance) {
        ...
        for(rawMaterialsUsed usedNTM:usedNTMs){
            RawMaterials ntmOpeningBalance=ntmsOpeningBalance.stream().filter(ntms->ntms.getId()==usedNTM.getId()).findAny().get();
    ...
    @Override
    public RawMaterials saveNTMs(RawMaterials ntMs) {
    ...
    @Override
    public boolean updateNTMs(Long id, RawMaterials ntMs) {
        RawMaterials details = ntMsRepository.findById(id)
    ...
    @Override
    public Collection<RawMaterials> ntmList(int limit) {
        return ntMsRepository.findAllNTMs(limit);
    }

    public Collection<rawMaterialsUsed> ntmsUsedList(LocalDate from, LocalDate to, int limit,boolean perProduct, Product product) {
        ArrayList<rawMaterialsUsed> ntMsFin = new ArrayList<>();
        List<RawMaterials> availableNTMs =ntMsRepository.findAllByOrderBySequenceAsc();
        ...
        for (RawMaterials nt : availableNTMs) {
            RawMaterials ntMs = nt;
            rawMaterialsUsed ntm= new rawMaterialsUsed();
    ...
    public Collection<rawMaterialsUsed> openingBalance(LocalDate endDate) {
        LocalDate startDate=LocalDate.parse("2023-08-01");
        Collection<rawMaterialsUsed> usedNTMs=this.ntmsUsedList(startDate, endDate, 1000000,false,new Product());
        List<RawMaterials> ntmsOpeningBalance=ntMsRepository.findAllByOrderBySequenceAsc();
    ...
    public Collection<completeNtmsUsed> completeNtmsUsed(LocalDate from, LocalDate to,Product product, int limit) {
        List<RawMaterials> allNTMs =new ArrayList<>();
        ...
        Collection<rawMaterialsUsed> ntmsClosingStock = this.ntmsList(limit);

        ArrayList<NTMsWithCountingIssues> countingIssues= new ArrayList<>();
        LocalDate ld=LocalDate.parse("2023-11-28");
        if(ld.isAfter(from) && ld.isBefore(to)) {
            log.info("we are here");
            for (RawMaterials ntMsFin : allNTMs) {
                NTMsWithCountingIssues ntMsWithCountingIssues = new NTMsWithCountingIssues();
        ...
        Collection<rawMaterialsUsed> availableNTMs =this.ntmsUsedList(from,to,limit,true,product);


        List<rawMaterialsUsed> ntmsOpeningBalance=new ArrayList<>(this.openingBalance(from));
        ...
        Collection <MaterialUsage> materialUsage = materialUsageServiceImplementation.materialUsageList(product.getId()); //producedProducts.stream().findFirst().get().getId()
        ...
        for (rawMaterialsUsed nt : availableNTMs) {
            ...
            for(rawMaterialsUsed ntd: ntmsClosingStock){
                if(ntd.getCode().equals(nt.getCode())){
    ...
    public Collection<NTMsRequiredExpected> ntmsRequiredExpectedList(int limit) {
        ...
        Collection<rawMaterialsUsed> availableNTMs = this.ntmsList(limit);
        ...
        for (rawMaterialsUsed ntMs : availableNTMs) {
            ...
            for (ProductionRun productionRun : productionRuns) {
                ...
                if (durationIndays > 0 || month.equals(month1)) {
                    ...
                    MaterialUsage materialUsage = materialUsageServiceImplementation.materialUsageList(productForProduction.getId())
                            .stream()
                            .filter(materialUsage1 -> materialUsage1.getRawMaterialUsage().getId().equals(ntMs.getId()))
                            .findAny().get();
