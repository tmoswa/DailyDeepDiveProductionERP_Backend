...
import com.softpala.SalesDeepDive_ERP.persistence.dao.FinishedProductMovementRepository;
...
public class FinishedProductMovementServiceImplementation implements iFinishedProductMovementService {
    ...
    public FinishedProductMovement saveFinishedProductMovement(FinishedProductMovement finishedProductMovement) {
        ...
        FinishedProductMovement saved = finishedProductMovementRepository.save(finishedProductMovement);

        TransferOutbox outbox = new TransferOutbox();
        outbox.setErpTransferId("ERP-TR-" + UUID.randomUUID());
        outbox.setProduct(product);
        outbox.setQty(saved.getQuantity());
        outbox.setTransferDate(saved.getMovement_date());
        outbox.setBatchNo(saved.getManual_reference());
        outbox.setStatus("APPROVED");
        transferOutboxRepository.save(outbox);
