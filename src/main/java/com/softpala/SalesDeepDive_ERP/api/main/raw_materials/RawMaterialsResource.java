...
public class RawMaterialsResource {
    ...
    @Autowired
    public RawMaterialsResource(RawMaterialsServiceImplementation ntMsServiceImplementation,
    ...
    public ResponseEntity<Response> registerNTMs(@RequestBody RawMaterials ntMs) {
        return  ResponseEntity.ok(
                Response.builder()
                        .timeStamp(now())
    ...
    @PutMapping(path = "{id}")
    @PreAuthorize("hasAuthority('PRIVILEGE-SUBSIDIARIES-ALTER')")
    public ResponseEntity<Response> updateNTM(@PathVariable("id") Long id,@RequestBody RawMaterials ntMs) {
