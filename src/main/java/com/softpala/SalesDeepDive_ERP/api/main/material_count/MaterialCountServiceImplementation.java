...
public class MaterialCountServiceImplementation implements iMaterialCountService {
    ...
    public MaterialCount saveMaterialCount(MaterialCount materialCount) {
        RawMaterials details=      ntMsRepository.findById(materialCount.getRawMaterialUsage().getId())
                .orElseThrow(() ->  new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("ntms with id %d not found", materialCount.getRawMaterialUsage().getId())));

        //details.setQuantity(materialCount.getQuantity());

        activityLogService.addActivityLog("Added Material Stock Count of : "+materialCount.getRawMaterialUsage().getName() +" , quantity is "+materialCount.getQuantity()+" , of Product: "+materialCount.getProduct_usage().getName(),"Material Stock Count");
    ...
    public boolean deleteMaterialCount(Long id) {
        ...
        activityLogService.addActivityLog("Deleted Material Count of : "+materialCount.getRawMaterialUsage().getName() +" , quantity "+materialCount.getQuantity()+" , of Product: "+materialCount.getProduct_usage().getName(),"Material Stock Count");
...
@Override
    ...
    if (details.getDescription().length() > 0) {
        ...
        RawMaterials NTMsDetails=      ntMsRepository.findById(materialCount.getRawMaterialUsage().getId())
                .orElseThrow(() ->  new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("ntms with id %d not found", materialCount.getRawMaterialUsage().getId())));
      ...
      //  details.setQuantity(quantityNew);
        ...
        activityLogService.addActivityLog("Updated Stock Count : "+materialCount.getRawMaterialUsage().getName() +" , from quantity "+details.getQuantity()+" , to quantity "+materialCount.getQuantity()+" , of Product: "+materialCount.getProduct_usage().getName(),"Material Stock Count");

        details.setRawMaterialUsage(materialCount.getRawMaterialUsage());
